...
(`ToInt32`, `Math.round`, `Math.hypot`, `toFixed`) the prototype leans on. All of it is ported
operation-for-operation so that a world generated on iOS, Android, the web or CI is identical to the
prototype's, bit for bit.

## Public API
| Item | Prototype | Notes |
|---|---|---|
| `hash2(x, y)` | `hash2` | `sin`-based; uses `libm::sin` |
...
- No `std` transcendental function is called (clippy `disallowed-methods`); only `libm`.
...
against V8). All comparisons are exact.
